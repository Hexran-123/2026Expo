...
function createOriginCard(screenElement) {
  ...
  function showPopularity(spot) {
    ...
    Popularity.record(spot.id).then((opens) => {
      ...
      // 段が上がるほど夜が明ける、犬吠埼の三段（夜／夜明け前／初日の出）。
      // 犬吠埼は本州最東端に近く、初日の出の名所として知られる場所であることから。
      // 灯りの強さではなく情景そのものを変えているので、3 段目だけが「一番美しい」。
      const icon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      icon.setAttribute('viewBox', '0 0 32 32');
      icon.setAttribute('width', '30');
      icon.setAttribute('height', '30');
      icon.setAttribute('aria-hidden', 'true');
      icon.innerHTML = popularityBadgeScene(level, `pop${popularityBadgeUid++}`);
  ...
  function popularityBadgeScene(level, uid) {
    if (level === 1) {
      // 夜。星と三日月の下、灯台が独りで灯る
      return `
        <defs>

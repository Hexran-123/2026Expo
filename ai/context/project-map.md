...
- 後端：**Supabase（絶景スポットの投稿と累積人気のためだけ。[ADR-0004](../../docs/adr/0004-投稿と集計のためにサーバーを持ち、ログインは持たない.md)）**。作品の中核データ（絶景スポット・時刻表・路線・地形）は今までどおり静的 JSON で、サーバーへは移さない。天候は気象庁 JSON API を直接叩く。**サーバーが落ちても作品は動く**（問い合わせは 3 秒で打ち切り、失敗したらその機能を出さないだけ）
- データベース：Supabase の PostgreSQL（投稿・累積人気）。端末側の永続化は `localStorage`（テーマ絞り込み・乗車区間・行動履歴）と IndexedDB（乗客が撮った写真。**投稿を押さないかぎり端末の外へ出ない**）
- 身分驗證：なし（**ログインは持たない**。投稿は匿名で受ける。ADR-0004）

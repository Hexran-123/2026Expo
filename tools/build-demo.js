 ...
 * 1 枚デモは demo/ に置くと決めてあるので、その前提で固定する。
 *
 ...
 *
 * 埋め込んでいない宛先（気象庁・Supabase）は、以前はここで断っていた。
 * しかし天気予報も累積人気も、本物のサイトでは動く機能であり、
 * 1 枚デモだけ違う挙動になるのは「本物と同じ規格」から外れる
 * （2026-08-13、累積人気が 1 枚デモでだけ出ないという指摘を受けて変更）。
 *
 * 路線データ・地形・時刻表は引き続き埋め込みから返す（1MB を超える頁を
 * 開いた直後に、同じデータをもう一度取りに行かせないため）。
 * それ以外は本物の fetch へそのまま通す——「動かないなら黙って隠す」の
 * 対象（設計書 9.3）であって、「デモだから止める」の対象ではない。
 */
...
/*
 * たたんだら、この段も一緒に引っこめる（2026-09-01）。

 * js/simulate.js はスマートフォンの幅（900px 未満）でたたんだ状態から始める。
 * 画面いっぱいにアプリが広がるので、開いたままだと下のものの上に乗るからである。
 * ところがこの段は .sim-body の外にあるため、たたんでも残り、操作盤の高さが
 * 200px ほどのままだった。その結果、地図の左下にある方位・縮尺と、
 * **地図の上に常時出している出典（.map-attrib）を覆っていた。**
 *
 * 出典は押さないと出ない形にしてはいけない（OSM の帰属ガイドライン。ADR-0003 の追記）。
 * デモの操作盤が覆っていては、常時出している意味がない。
 * 路線と動かし方は、操作盤を開けばこれまでどおり替えられる。
 */
.sim--folded .demo-switch { display: none; }

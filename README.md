...
- **実演用の路線（有楽町線）のデータが重い**。`terrain.json` 236KB・`schedule.json` 322KB
  （実際のダイヤに差し替えたぶん、測定時点の 185KB より増えている）で、
  `demo/all.html` の一部がこれ。**応募時も載せたまま出す方針に変わったので、
  「外すから関係しない」とは言えなくなった。** ただし払うのは有楽町線を選んだ人だけで、
  路線選択画面が読むのは `preview.json`（銚子電鉄 13.6KB・有楽町線 34.6KB）に限られる。
  効き目が大きいのは `terrain.json` の輪郭を間引くこと（銚子電鉄は 6.4km ぶんで 38KB、
  有楽町線は 28.4km ぶんを実機テスト用に間引かず作ってある）。
- **`js/main.js` が 216KB（gzip 68.5KB）**。届くまで地図を描き始められないので、ここがいまの頭打ち。
  車上モード・旅の記録などを足すたびに大きくなってきており、下の比較表を測ったとき（142KB・
  JS 6 本合計 gzip 69.3KB）より 1 本ぶんだけで既にその水準を超えている。数値は測り直しが要るが、
  結論（分けるほうはやらない）は変わらない。

  | | 効き目 | 代償 |
  |---|---|---|
  | **minify（ビルド工程）** | JS 6 本が gzip 69.3KB → **24.3KB**（測定時点。いまはこれより増えている）。地図まで 6%（銚子）〜23%（路線選択）速い | 作り直し忘れが起きる（直したのに古いものが配信され、エラーも出ない）。現地でエラーが出たとき行番号も変数名も潰れている |
...
- [x] 旅の記録（設計書 7）通過の記録・写真・共有用の縦長画像・SNS への共有（対応端末のみ）
...
- [x] **成因カードの中身** ── `data/choshi/spots.json`・`data/yurakucho/spots.json` とも
      全スポットの `panels` を書いた（銚子電鉄 6 件・有楽町線 10 件）。銚子電鉄は 1 コマ 200〜300 字台を
      3〜5 コマ、有楽町線は 1 コマ 80 字以内を 5 コマに分けている。画像は任意で、いまは銚子電鉄の S04 以外に 1 枚ずつ入っている
...
- [ ] 現地での確かめ（[将来構想 6](docs/将来構想.md)）
      ...
      （2026-08-20 に現地調査を予定。[チェックリスト](docs/銚子電鉄_現地調査チェックリスト_2026-08-20.docx)）

位置情報を断っても地図はそのまま使える。発車待ちと車上モードが出なくなるだけ。

`panels` が空のスポットでは、通過しても成因カードを出さない（空のカードを出さないため）。
新しいスポットを足した直後や、`panels` をまだ書いていない路線ではこの既定に落ちる。
